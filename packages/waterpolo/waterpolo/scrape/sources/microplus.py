...
class MicroPlusSource:
    ...
    def fetch_calendar_file(self, calendar_url: str) -> List[Any]:
        payload = self.client.get_json(calendar_url)
        return parse_calendar(payload)
    ...
    def fetch_contatori(self) -> Dict[str, List[Dict[str, str]]]:
        endpoint = self.endpoints.get("contatori")
        if not endpoint:
            raise ValueError("Contatori endpoint not configured")
        payload = self.client.get_json(endpoint)
        return parse_contatori(payload)
